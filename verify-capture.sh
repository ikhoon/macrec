...

{
  ...
  if [[ -x "$WHISPER_CLI" && -f "$WHISPER_GGML" && -s "$OUT.wav" ]]; then
    ...
    echo "(whisper-cli/model missing — skipping transcript)"
  fi
  echo
