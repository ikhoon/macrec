#!/bin/zsh
# package.sh — build a SELF-CONTAINED, self-signed MeetingRecorder.app for DISTRIBUTION.
#
...
# Output: dist/MeetingRecorder.zip  — unzip, drag to /Applications, launch.
set -e
HERE="${0:A:h}"
...
echo "▸ zipping…"
( cd "$DIST" && rm -f MeetingRecorder.zip && ditto -c -k --sequesterRsrc --keepParent "MeetingRecorder.app" "MeetingRecorder.zip" )

echo
echo "✅ 배포물: $DIST/MeetingRecorder.zip  ($(du -h "$DIST/MeetingRecorder.zip" | cut -f1))"
echo "   포함: 앱 + self-contained whisper-cli + silero VAD.  (큰 모델은 첫 실행에 다운로드)"
echo
echo "👉 유저 설치:"
echo "   1) zip 풀고 MeetingRecorder.app 을 /Applications 로 드래그"
