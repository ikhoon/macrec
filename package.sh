#!/bin/zsh
# package.sh — build a SELF-CONTAINED, self-signed Amanu.app for DISTRIBUTION.
#
...
# Output: dist/Amanu.zip  — unzip, drag to /Applications, launch.
set -e
HERE="${0:A:h}"
...
echo "▸ zipping…"
( cd "$DIST" && rm -f Amanu.zip && ditto -c -k --sequesterRsrc --keepParent "Amanu.app" "Amanu.zip" )

echo
echo "✅ 배포물: $DIST/Amanu.zip  ($(du -h "$DIST/Amanu.zip" | cut -f1))"
echo "   포함: 앱 + self-contained whisper-cli + silero VAD.  (큰 모델은 첫 실행에 다운로드)"
echo
echo "👉 유저 설치:"
echo "   1) zip 풀고 Amanu.app 을 /Applications 로 드래그"
