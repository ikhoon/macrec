...
enum Notifier {
    ...
    /// `openWindow` ("log" / "today") routes a click to an in-app window instead — so an informational
    /// alert (e.g. the #27 outage) has a real destination rather than being a dead click.
    static func push(title: String, body: String, filePath: String? = nil, openURL: URL? = nil,
                     openWindow: String? = nil) {
        ...
        if let openWindow { info["open"] = openWindow }           // an in-app window to bring forward on click
